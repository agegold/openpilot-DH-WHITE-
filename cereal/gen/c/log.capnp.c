...
void cereal_read_ControlsState(struct cereal_ControlsState *s, cereal_ControlsState_ptr p) {
	...
	s->cruiseSetMode = capn_read32(p.p, 176);
	s->alertTextMsg1 = capn_get_text(p.p, 6, capn_val0);
	s->alertTextMsg2 = capn_get_text(p.p, 7, capn_val0);
	s->pCurvature = capn_to_f32(capn_read32(p.p, 180));
	s->curvMaxSpeed = capn_to_f32(capn_read32(p.p, 184));
	s->targetSpeed = capn_to_f32(capn_read32(p.p, 188));
	s->longWaitCmd = capn_to_f32(capn_read32(p.p, 192));
	s->longTimerCmd = capn_to_f32(capn_read32(p.p, 196));
...
void cereal_write_ControlsState(const struct cereal_ControlsState *s, cereal_ControlsState_ptr p) {
	...
	capn_write32(p.p, 176, s->cruiseSetMode);
	capn_set_text(p.p, 6, s->alertTextMsg1);
	capn_set_text(p.p, 7, s->alertTextMsg2);
	capn_write32(p.p, 180, capn_from_f32(s->pCurvature));
	capn_write32(p.p, 184, capn_from_f32(s->curvMaxSpeed));
	capn_write32(p.p, 188, capn_from_f32(s->targetSpeed));
	capn_write32(p.p, 192, capn_from_f32(s->longWaitCmd));
	capn_write32(p.p, 196, capn_from_f32(s->longTimerCmd));
...
void cereal_read_Plan(struct cereal_Plan *s, cereal_Plan_ptr p) {
	...
	s->pCurvature = capn_to_f32(capn_read32(p.p, 76));
	s->curvMaxSpeed = capn_to_f32(capn_read32(p.p, 80));
}
void cereal_write_Plan(const struct cereal_Plan *s, cereal_Plan_ptr p) {
	...
	capn_write32(p.p, 76, capn_from_f32(s->pCurvature));
	capn_write32(p.p, 80, capn_from_f32(s->curvMaxSpeed));
}
...
void cereal_read_Event(struct cereal_Event *s, cereal_Event_ptr p) {
	...
	case cereal_Event_sentinel:
		s->sentinel.p = capn_getp(p.p, 0, 0);
		break;
	default:
		break;
	}
