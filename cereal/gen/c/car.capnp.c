...
void cereal_read_CarState(struct cereal_CarState *s, cereal_CarState_ptr p) {
	...
	s->distanceToggle = capn_to_f32(capn_read32(p.p, 44));
	s->laneDepartureToggle = (capn_read8(p.p, 48) & 1) != 0;
	...
	s->readdistancelines = capn_to_f32(capn_read32(p.p, 52));
	s->lkMode = (capn_read8(p.p, 48) & 2) != 0;
	s->lcaLeft = (capn_read8(p.p, 48) & 16) != 0;
	s->lcaRight = (capn_read8(p.p, 48) & 32) != 0;
	s->canMonoTimes.p = capn_getp(p.p, 4, 0);
	s->leftBlindspot = (capn_read8(p.p, 48) & 4) != 0;
	s->rightBlindspot = (capn_read8(p.p, 48) & 8) != 0;
}
void cereal_write_CarState(const struct cereal_CarState *s, cereal_CarState_ptr p) {
	...
	capn_write32(p.p, 44, capn_from_f32(s->distanceToggle));
	capn_write1(p.p, 384, s->laneDepartureToggle != 0);
	...
	capn_write32(p.p, 52, capn_from_f32(s->readdistancelines));
	capn_write1(p.p, 385, s->lkMode != 0);
	capn_write1(p.p, 388, s->lcaLeft != 0);
	capn_write1(p.p, 389, s->lcaRight != 0);
	capn_setp(p.p, 4, s->canMonoTimes.p);
	capn_write1(p.p, 386, s->leftBlindspot != 0);
	capn_write1(p.p, 387, s->rightBlindspot != 0);
}
...
void cereal_read_CarParams(struct cereal_CarParams *s, cereal_CarParams_ptr p) {
	...
	s->mdpsBus = (int8_t) ((int8_t)capn_read8(p.p, 82));
	s->sasBus = (int8_t) ((int8_t)capn_read8(p.p, 83));
	s->sccBus = (int8_t) ((int8_t)capn_read8(p.p, 84));
	s->autoLcaEnabled = (int8_t) ((int8_t)capn_read8(p.p, 85));
}
void cereal_write_CarParams(const struct cereal_CarParams *s, cereal_CarParams_ptr p) {
	...
	capn_write8(p.p, 82, (uint8_t) (s->mdpsBus));
	capn_write8(p.p, 83, (uint8_t) (s->sasBus));
	capn_write8(p.p, 84, (uint8_t) (s->sccBus));
	capn_write8(p.p, 85, (uint8_t) (s->autoLcaEnabled));
}
